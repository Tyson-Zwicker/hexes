...
export default class HexGrid {
  constructor(hexSize = 30, orientation = 'flat') {
    this.hexSize = hexSize; // Size of each hexagon
    this.orientation = orientation; // 'flat' or 'pointy'
  }

  // 1. Return a list of neighbors within a given radius
  ...
  // 2. Return distance to another grid location
  ...
  // 3. Calculate shortest direction to another grid location
  getDirection(q1, r1, q2, r2) {
    ...
    // Get the direction vector (normalized to unit hex steps)
    const directions = [
      { q: 1, r: 0 },   // Right
      { q: 1, r: -1 },  // Up-Right
      { q: 0, r: -1 },  // Up-Left
  ...
  // 5. Convert screen coordinates to hex grid coordinates
  ...
  // 4. Draw method - draws a single hex at the given grid coordinates
  // centerQ, centerR: the hex coordinates that should be centered on screen
  // canvasWidth, canvasHeight: dimensions of the canvas for centering
  ...
  // centerQ, centerR: the hex coordinates that should be centered on screen
  drawGrid(ctx, qMin, qMax, rMin, rMax, centerQ, centerR, canvasWidth, canvasHeight, fillColor = '#ffffff', strokeColor = '#44444') {
    for (let q = qMin; q <= qMax; q++) {
      for (let r = rMin; r <= rMax; r++) {
        console.log (q,r);
        this.drawHex(ctx, q, r, centerQ, centerR, canvasWidth, canvasHeight, fillColor, strokeColor);
      }
    }
  }

  // Helper method to get the corner points of a hex at pixel coordinates
  ...
  // Optional: Helper to draw text in the center of a hex
  // centerQ, centerR: the hex coordinates that should be centered on screen
